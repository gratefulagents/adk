...
func writerFixture() {
	...
	must(err)
	result := map[string]any{"request_json": string(requestBytes), "response_json": string(responseBytes), "native_request_cases": nativeRequestCases(), "nonfinite_span_cases": nonFiniteSpanCases(store), "routing_settings": routingSettingsCases()}
	var variants []string
