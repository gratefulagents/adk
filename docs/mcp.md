...
authentication. Header values and raw remote errors are never diagnostic output.
...
Tool requests include a length-delimited SHA-256 of tenant, tool and serialized
arguments for approval/audit binding. Callbacks must apply the same guardrails,
...
`fixtures/mcp`. Fixtures are source-backed expectations from SDK MCP tests, not a
claim that a live Go differential runner was executed.
